...
name: humanizer
description: >-
  Use before any prospect, client, guest, public, Instantly, Slack, email, or
  agent-to-agent message ships. Removes AI writing tells (blader/humanizer /
  Wikipedia Signs of AI writing). Fleet hard rule: never use em dashes or en
  dashes in any agent communication.
---

# Fleet hard rules (Ty / ClawRevOps)

These override everything below when they conflict:

1. **No em dashes (—) and no en dashes (–) in any agent communication. Ever.** That includes Instantly copy, Slack, email, LinkedIn, agent-to-agent messages, and user-facing chat. Prefer periods, commas, parentheses, or a short rewrite. Double hyphens used as dashes (` -- `) are also banned outside code.
2. **Normalize sentence patterns to sound human.** Vary length. Skip chatbot wrappers, forced triads, not-X-but-Y staging, sales inflation, and decorative bold.
3. **Outbound QC still applies.** After humanizing, run Draft → leak-scan → VIP spot-check → then upload/send.

Source: [blader/humanizer](https://github.com/blader/humanizer) (MIT). Keep the upstream patterns below.
